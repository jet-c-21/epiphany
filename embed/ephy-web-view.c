...
on_document_loaded_cb (WebKitWebView *web_view)
{
  ...
  /* FIXME: filter out about:* pages and all other internal (albeit they are HTML too) pages. ephy_embed_utils_url_is_empty() and/or ephy_embed_utils_is_no_show_address() ?*/
  if (view->document_type == EPHY_WEB_VIEW_DOCUMENT_HTML
      /* FIXME: those three checks are kinda ridiculous at this point. Is there a "this is a real web page" check that covers them all? => also look at mentioned functions just above */
      && !ephy_web_view_get_is_blank (view) && !ephy_web_view_is_newtab (view) && !ephy_web_view_is_overview (view)) {
