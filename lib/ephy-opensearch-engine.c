                 ...
                 GError     **error)
{
  ...
  if (!after_scheme) {
    ...
    *error = g_error_new (G_IO_ERROR, G_IO_ERROR_FAILED,
                          _("The %s search engine template URL isn't a proper URL?!"),
                          url);
    return NULL;
  }
                      ...
                      GError              **out_error)
{
   ...
   */
    ...
    if (!param_name || !param_value) {
      if (out_error)
        *out_error = g_error_new (G_IO_ERROR, G_IO_ERROR_FAILED, "Skipping <Param> tag as it has no name or value attribute");
      return;
    }
             ...
             GError              **error)
{
   ...
   */
  char *stripped_text = g_strstrip (original_text);
                                  ...
                                  gpointer      user_data)
{
  ...
  /* We don't use autoptr because the task will take ownership of the error. */
  GError *error = NULL;
