                              ...
                              gpointer      user_data)
{
  ...
  for (guint i = 0; i < suggestions_count; i++) {
     ...
     */
    unescaped_title = g_strdup_printf (_("%s — %s"), suggestion_term, ephy_search_engine_get_name (engine));
    ...
    suggestion = ephy_suggestion_new (escaped_title, unescaped_title, suggestion_address);
