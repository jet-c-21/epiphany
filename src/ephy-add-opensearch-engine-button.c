...
G_DEFINE_FINAL_TYPE (EphyAddOpensearchEngineButton, ephy_add_opensearch_engine_button, ADW_TYPE_BIN)
...
{
  EphySearchEngineRow *row = EPHY_SEARCH_ENGINE_ROW (object);
  g_signal_connect (row, "fully-expanded", G_CALLBACK (row_expanded_cb), NULL);
...
{
  AdwPreferencesWindow *prefs_dialog = ADW_PREFERENCES_WINDOW (ephy_shell_get_prefs_dialog (ephy_shell_get_default ()));
   ...
   */
  adw_preferences_window_set_visible_page_name (prefs_dialog, "general_page");
  general_page = EPHY_PREFS_GENERAL_PAGE (adw_preferences_window_get_visible_page (prefs_dialog));
...
on_opensearch_engine_loaded_cb (EphyOpensearchAutodiscoveryLink *autodiscovery_link,
                                ...
                                gpointer                         user_data)
{
  g_autofree struct LoadEngine *data = user_data;
                              ...
                              gpointer user_data)
{
  ...
  g_autofree char *label_str =
    g_strdup_printf (_("Add <b>%s</b> as search engine."), escaped_name);
...
ephy_add_opensearch_engine_button_class_init (EphyAddOpensearchEngineButtonClass *klass)
{
  ...
  gtk_widget_class_bind_template_callback (widget_class, on_opensearch_row_activated_cb);
  gtk_widget_class_bind_template_child (widget_class, EphyAddOpensearchEngineButton, list_box);
  gtk_widget_class_bind_template_child (widget_class, EphyAddOpensearchEngineButton, menu_button);
}
