                                ...
                                EphySearchEngineListBox *self)
{
  ...
  while ((row = gtk_list_box_get_row_at_index (GTK_LIST_BOX (self->list), i++))) {
    ...
    if (ADW_EXPANDER_ROW (row) == expanded_row)
      continue;
                          ...
                          gpointer user_data)
{
  ...
  ephy_search_engine_row_set_radio_button_group (row, GTK_CHECK_BUTTON (self->radio_buttons_group));
  ...
  if (self->is_model_initially_loaded) {
     ...
     * to connect to the "fully-expanded" signal because the animation will not start
